...
medcsv = pd.read_csv('mimicData/P3/inputText22161587.csv')
# uniikit lääkenimet valintaa varten
medoptions = sorted(medcsv["label"].unique())
# charted event taulukko
charcsv = pd.read_csv('mimicData/P3/Responce22151687.csv')
...
# diagnoosit taulukko
diagcsv = pd.read_csv('mimicData/P3/patient22161587_descDiag.csv')
...
# Medication graph
@callback(
    Output("medication3", "figure"),
    [Input("medChecklist3", "value")])
...
@callback(
    Output("chartedevents3", "figure"),
    [Input("charChecklist3", "value")])
