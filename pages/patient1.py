...
medcsv = pd.read_csv('mimicData/P1/inputText066.csv')
# uniikit lääkenimet valintaa varten
medoptions = sorted(medcsv["label"].unique())
# charted event taulukko
charcsv = pd.read_csv('mimicData/P1/Responce24843066.csv')
...
# diagnoosit taulukko
diagcsv = pd.read_csv('mimicData/P1/patient066_descDiag.csv')
...
# Medication graph
@callback(
    Output("medication", "figure"),
    [Input("medChecklist", "value")])
...
@callback(
    Output("chartedevents", "figure"),
    [Input("charChecklist", "value")])
