...
medcsv = pd.read_csv('mimicData/P2/inputText20633855.csv')
# uniikit lääkenimet valintaa varten
medoptions = sorted(medcsv["label"].unique())
# charted event taulukko
charcsv = pd.read_csv('mimicData/P2/Responce20633855.csv')
...
# diagnoosit taulukko
diagcsv = pd.read_csv('mimicData/P2/patient20633855_descDiag.csv')
...
# Medication graph
@callback(
    Output("medication2", "figure"),
    [Input("medChecklist2", "value")])
...
@callback(
    Output("chartedevents2", "figure"),
    [Input("charChecklist2", "value")])
